...
class CostumerModel {
  ...
  static CostumerModel? get stored => SystemCache.get('costumers');

  void setUser(CostumerModel costumer) =>
      SystemCache.set('costumers', costumer);
  static void _setUser(CostumerModel? costumer) =>
      SystemCache.set('costumers', costumer);

  static void _deleteUser() => SystemCache.remove('costumers');
  ...
  static Stream<CostumerModel> stream() {
    return SystemMDBService.db.collection('costumers').find().transform(
  ...
  Future<CostumerModel?> aggregate(List<dynamic> pipeline) async {
    var d =
        await SystemMDBService.db.collection('costumers').aggregate(pipeline);
  ...
  static Future<CostumerModel?> findByUsername(String username) async {
    ...
    // var s = SystemMDBService.db.collection('costumers').find();
    // print(await s.first);
    print(r);
    if (r == null) {
      return null;
  ...
  Future<CostumerModel?> edit() async {
    var r = await SystemMDBService.db.collection('costumers').update(
          where.eq('id', id),
          toMap(),
        );
    print('User Update: $r');
  ...
  Future<int> delete(String id) async {
    var r = await SystemMDBService.db.collection('costumers').remove(
          where.eq('id', id),
        );
    print(r);
    return 1;
  ...
  Future<CostumerModel> add() async {
    var r = await SystemMDBService.db.collection('costumers').insert(
          toMap(),
        );
    print(r);
