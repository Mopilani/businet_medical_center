...
class SupplierModel {
  ...
  Stream<SupplierModel> stream() {
    return SystemMDBService.db.collection('suppliers').find().transform(
      StreamTransformer.fromHandlers(
        handleData: (data, sink) {
          sink.add(SupplierModel.fromMap(data));
        },
  ...
  Future<SupplierModel?> aggregate(List<dynamic> pipeline) async {
    var d =
        await SystemMDBService.db.collection('suppliers').aggregate(pipeline);
  ...
  Future<SupplierModel> edit() async {
    var r = await SystemMDBService.db.collection('suppliers').update(
          where.eq('id', id),
          toMap(),
        );
    print(r);
  ...
  Future<int> delete() async {
    var r = await SystemMDBService.db.collection('suppliers').remove(
          where.eq('id', id),
        );
    print(r);
    return 1;
  ...
  Future<int> add() async {
    var r = await SystemMDBService.db.collection('suppliers').insert(
          toMap(),
        );
    print(r);
    return 1;
